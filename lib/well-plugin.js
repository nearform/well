...
function rand(bound) {
  return bound * Math.floor(Math.abs(Math.random()))
}
...
function WellPlugin() {
  ...
  self.createvent = function(args,cb){
  ...
  self.register = function(args,cb){
    args.parent$(args,si.err(cb,function(user){
  ...
  self.getuser = function(args,cb){
    ...
    var term
    _.find(['id','nick','email','twid'],function(t){return term==args[t]})

    if( term ) {
      var q = {}
      q[term]=args[term]
      userent.load$(q,si.err(function(user){
        if( user && eventid ) {
          loadevent(eventid,cb,function(event){
  ...
  self.well = function(args,cb){
    ...
    loadevent(eventid,cb,function(event){
      ...
      teament.load$({event:eventid,num:usermeta.t},si.err(function(team){
  ...
  self.init = function(init_si,init_opts,cb){
    ...
    seneca.add({role:'user',cmd:'register'},    self.register)

    seneca.add({role:self.name,cmd:'createevent'}, self.createevent)
    seneca.add({role:self.name,cmd:'joinevent'},   self.joinevent)
    seneca.add({role:self.name,cmd:'getteam'},     self.getteam)
    seneca.add({role:self.name,cmd:'getuser'},     self.getuser)
    seneca.add({role:self.name,cmd:'well'},        self.well)

    cb()
  }


A REPL WOULD BE NICE
