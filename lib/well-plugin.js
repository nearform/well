...
function WellPlugin() {
  ...
  self.service = function() {
    function herr(res,win) {
      return function(err) {
        if( err ) { return res.send(500,err) }
        win && win.apply(null,Array.prototype.slice.call(arguments,1))
      }
    }
    ...
    return dispatch({
      '/well': {
        GET: {
          '/user/:nick': function(req,res,next,nick) {
            self.getuser({nick:nick},herr(res,function(user){
              if( !user ) return res.send(404,'not found');

              var saneuser = sanitizeuser(user)
              console.dir(saneuser)
              res.send(saneuser)
            }))
          }
        }
